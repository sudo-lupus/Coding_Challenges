...
def gta(limit, *args):
    ...
    for i in range(max(map(len, args_list))):
        for n in range(len(args_list)):
            try:
                if args_list[n][i] in base_list: continue
                if len(base_list) >= limit: break
                else: base_list.append(args_list[n][i])
            except IndexError:
                continue
    sum_n = 0
    base_list = list(map(int, base_list))
    for i in range(1,limit+1):
        sum_n += sum(list(map(sum, (permutations(base_list, i)))))
    return sum_n